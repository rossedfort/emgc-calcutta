<script lang="ts">
	...
	onMount(() => {
		const { data: authListener } = supabase.auth.onAuthStateChange(async (_event, newSession) => {
			...
			// layout's own load, dropping AppShell, but leaves whatever page
			// happened to already be mounted rendering its own
			// never-invalidated (now-stale) data on top of nothing. Every
			// route except /login requires a session, so bouncing there
			// unconditionally mirrors the redirect(303, '/login') every
			// protected route's own server load already does — this just
			// covers the case where no fresh navigation/SSR happens on its
			// own for a tab that's just sitting open.
