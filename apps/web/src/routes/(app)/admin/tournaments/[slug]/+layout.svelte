<script lang="ts">
	...
	// on that page.
	let settingsPath = $derived(routes.adminTournament(data.tournament.slug));
...
<div class="flex flex-col gap-4">
	<PageHeader title={data.tournament.name} eyebrow="Admin">
		{#snippet actions()}
			<a href={routes.adminTournaments()} class="text-sm text-brass hover:underline"
		...
		tabs={[
			{
				...
				label: 'Settings',
				exact: true,
				matchPrefix: routes.adminTournamentEdit(data.tournament.slug)
			},
			{
			...
			{
				href: routes.adminTournamentResults(data.tournament.slug),
				label: 'Results'
			},
			{
				href: routes.adminTournamentBookkeeping(data.tournament.slug),
				label: 'Bookkeeping'
			}
		]}
	/>
