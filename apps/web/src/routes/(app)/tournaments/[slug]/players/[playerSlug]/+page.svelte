...
<div class="flex flex-col gap-4">
	<PageHeader title={formatPlayerName(data.player)}>
		{#snippet actions()}
			<a href={routes.tournament(data.tournament.slug)} class="text-sm text-brass hover:underline"
				>Back to players</a
			>
		{/snippet}
	</PageHeader>
				...
				{:else}
					<ul class="flex flex-col gap-1 text-sm">
						{#each data.pooledPlayers as pooled (pooled.slug)}
							<li>
								<a
									href={routes.tournamentPlayer(data.tournament.slug, pooled.slug)}
	...
	{#each data.entries as entry (entry.id)}
		<div class="rounded-lg border border-brass/30 bg-scorecard p-6 text-ink">
			<div class="flex items-center gap-2">
				...
				{#if entry.fieldEntry}
					<a
						href={routes.tournamentPlayer(data.tournament.slug, entry.fieldEntry.slug)}
