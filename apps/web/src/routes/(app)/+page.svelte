...
<div class="flex flex-col gap-6">
	{#each tournaments as tournament (tournament.id)}
		...
		<div class="rounded-lg border border-brass/30 bg-scorecard p-8 text-ink shadow-sm">
			...
			<div class="mt-6 flex items-center gap-2">
				<Button href={routes.tournament(tournament.slug)} variant={isActive ? 'fairway' : 'brass'}>
					{isActive ? 'Join Auction' : 'View the field'}
				</Button>
				{#if isAdmin}
					<Button href={routes.adminTournament(tournament.slug)} variant="brass"
	...
	{:else}
		<div class="rounded-lg border border-brass/30 bg-scorecard p-8 text-ink shadow-sm">
			...
			{#if isAdmin}
				<div class="mt-6">
					<Button href={routes.adminTournamentNew()} variant="brass">Create tournament</Button>
				</div>
			{/if}
		</div>
	{/each}
					...
					{#each pastTournaments as tournament (tournament.id)}
						<Table.Row>
							<Table.Cell class="font-medium text-ink">
								<a href={routes.tournamentResults(tournament.slug)} class="hover:underline">
									{tournament.name}
								</a>
							</Table.Cell>
