...
-- (0-1 range, e.g. 0.5 for 50%) at the time this row was calculated — kept
-- alongside the derived dollar amount so a later payout_structure edit
-- can't retroactively change what an already-calculated Payout looked
-- like. amount numeric(10,2) matches bids.amount's own money precision.
...
-- page) plus Admin/Owner read-all (feeds the bookkeeping dashboard) — same
-- "self or role" shape as users_select_self_or_admin, just matched against
-- bidder_id instead of id.
--
...
-- get a plain-RLS write policy for simple Admin state changes), nothing
-- about Payout is ever meant to be a direct client write.
...
-- project (see .claude/CLAUDE.md Known quirks) — required for the
